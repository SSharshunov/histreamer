...
#define THIS_FILE	"video_stream_util.c"


/* If set, local renderer will be created to play original file */
#define HAS_LOCAL_RENDERER_FOR_PLAY_FILE    0
...
int some_hex_string_to_octet_string(char *raw, char *hex, int len)
{
    int i;
    for (i = 0; i < len; i+=2) {
	int tmp;
...
#if defined(PJMEDIA_HAS_H264_HISILICON_PASSTHROUGH_VID_CODEC) && PJMEDIA_HAS_H264_HISILICON_PASSTHROUGH_VID_CODEC != 0
    status = pjmedia_codec_h264_hisilicon_passthrough_vid_init(NULL, pf);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
#endif
...
#if defined(PJMEDIA_HAS_H264_HISILICON_PASSTHROUGH_VID_CODEC) && PJMEDIA_HAS_H264_HISILICON_PASSTHROUGH_VID_CODEC != 0
    pjmedia_codec_h264_hisilicon_passthrough_vid_deinit();
#endif
...
int vidstreammain(int argc, char *argv[])
{
	...
	case OPT_SRTP_TX_KEY:
	    tmp_key_len = some_hex_string_to_octet_string(tmp_tx_key, pj_optarg, 
						        (int)strlen(pj_optarg));
	    pj_strset(&srtp_tx_key, tmp_tx_key, tmp_key_len/2);
	    break;

	case OPT_SRTP_RX_KEY:
	    tmp_key_len = some_hex_string_to_octet_string(tmp_rx_key, pj_optarg,
						        (int)strlen(pj_optarg));
    ...
    PJ_LOG(3, (THIS_FILE, "pjmedia_vid_codec_mgr_get_default_param PJ_SUCCESS"));
	...
	codec_param.enc_fmt.det.vid.fps  = file_vfd->fps;
        ...
        PJ_LOG(3, (THIS_FILE, "Set as active for all video devices PJ_SUCCESS"));
	/* Create video device port. */
        if (dir & PJMEDIA_DIR_ENCODING) {
            PJ_LOG(3, (THIS_FILE, "PJMEDIA_DIR_ENCODING"));
            /* Create capture */
        ...
        if (dir & PJMEDIA_DIR_DECODING) {
            PJ_LOG(3, (THIS_FILE, "PJMEDIA_DIR_DECODING"));
            /* Create renderer */
            status = pjmedia_vid_dev_default_param(
					pool,
